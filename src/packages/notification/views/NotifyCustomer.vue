...
import ModalCreateNotifyCustomer from '@/packages/notification/components/ModalCreateNotifyCustomer'
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      const result = await this[FETCH_LIST_NOTIFY_CUSTOMER](this.filter)
      if (!result.success) {
    ...
    async handleSaveNotifyEmail(payload) {
      ...
      const result = await this[CREATE_NOTIFY_CUSTOMER](payload)
      this.isSubmitting = false
      if (!result.success) {
        this.$toast.open({
          type: 'error',
